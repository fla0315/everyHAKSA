...
<script type="text/javascript">
   function boardList(curPage) {
      ...
      $('form[name=frmPage]').submit();
       /* frmPage.action="<c:url value='/admin/product/productList'/>";
        frmPage.submit(); */
   }
   
   $(function(){
      $('input[name=chkAll]').change(function(){
         $('.divList table tbody input[type=checkbox]').prop('checked', this.checked);
      });
   });
...
<!-- 페이징 처리를 위한 form 시작-->
<form name="frmPage" method="post" action="<c:url value='/admin/product/productList'/>">
...
<c:if test="${empty param.eventName }">
   <p> 전체조회 ${pagingInfo.totalRecord } 건 입니다.</p>
</c:if>   
<c:if test="${!empty param.eventName }">
   <p> ${param.eventName } 상품 ${pagingInfo.totalRecord } 건 입니다.</p>
</c:if>


<form name="frmList" method="post" action="<c:url value='/admin/product/productList'/>">
<div class="divRight">   
   이벤트 상품 조회
   <select name="eventName" >
      <option value=""></option>
      <option value="NEW" <c:if test="${param.eventName=='NEW'}"> selected="selected" </c:if> >신상품</option>
      <option value="BEST" <c:if test="${param.eventName=='BEST'}"> selected="selected" </c:if> >인기상품</option>
      <option value="MD" <c:if test="${param.eventName=='MD'}"> selected="selected" </c:if> >MD추천상품</option>            
   </select>
      ...
      <c:if test="${!empty list }">
         ...
         <c:forEach var="vo" items="${list}">
            <tr>
               ...
               <td>
                  <img src ="<c:url value='/pd_images/${vo.imageUrl }'/>" width="70">
               </td>
               <td>${vo.productName }</td>
               <td><fmt:formatNumber value='${vo.sellPrice }' pattern="#,###원"/></td>
